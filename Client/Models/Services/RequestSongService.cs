...
namespace YukariBlazorDemo.Client.Models.Services
{
	public class RequestSongService : ApiService
	{
		...
		// --------------------------------------------------------------------
		// 予約一覧を取得
		// ＜返値＞ (成功した場合は空文字列、エラーの場合はエラーメッセージ, 予約 1 ページ分, 総数)
		// --------------------------------------------------------------------
		public async Task<(String, RequestSong[], Int32)> GetRequestSongsAsync(String? query)
		{
			(HttpStatusCode statusCode, RequestSong[] reqestSongs, Int32 totalCount) = await GetArrayFromJsonAsync<RequestSong>(YbdConstants.URL_REQUEST, query);
			return (DefaultErrorMessage(statusCode), reqestSongs, totalCount);
		}

		// --------------------------------------------------------------------
		// 予約者名一覧を取得
		// ＜返値＞ (成功した場合は空文字列、エラーの場合はエラーメッセージ, 予約者 1 ページ分, 総数)
		// --------------------------------------------------------------------
		public async Task<(String, String[], Int32)> GetUserNamesAsync()
		{
			(HttpStatusCode statusCode, String[] names, Int32 totalCount) = await GetArrayFromJsonAsync<String>(YbdConstants.URL_GUEST_USER_NAMES);
